...
def main():
    ...
    parser.add_argument('--tag_FT', default='aliceDS0_LLM_phi4k_10ep_normRNA_normPro',
                        help='tag to be used to store the fine-tuned model')
    parser.add_argument('--tag_test', default='aliceDS0_LLM_phi4k_10ep_normRNA_normPro_test_normRNA_normPro',
    ...
    parser.add_argument('--llm_model', default="microsoft/Phi-3-mini-4k-instruct",
    # parser.add_argument('--llm_model', default="microsoft/Phi-3-mini-128k-instruct",
                        help='Which LLM to use. Supply the path for hugging face automodel that is compatible with AutoModelForCausalLM, remote or local path')    
    ...
    #--- Prepare The Model ---#
    ###########################
    tokeniser = AutoTokenizer.from_pretrained(args.llm_model)
    model = AutoModelForCausalLM.from_pretrained(args.llm_model, trust_remote_code=True, torch_dtype = torch.bfloat16)
    
    peft_config = LoraConfig(task_type=TaskType.CAUSAL_LM, target_modules=["qkv_proj"], inference_mode=False, r=16, lora_alpha=32, lora_dropout=0.05)
    model = get_peft_model(model, peft_config)

    # Resume training from breakpoints
    if args.resume == True:
        sys.exit("Resume option is not yet implemented!")
        ...
        #---  Prepare optimiser ---#
        optimiser = optim.Adam(model.parameters(), lr=args.lr, amsgrad=True) 
        optimiser.load_state_dict(checkpoint['optimiser'])
        #---  Prepare Scheduler ---#
        scheduler = StepLR(optimiser, step_size=args.gamma_step, gamma=args.gamma)
    ...
    for epoch in range(start_epoch+1, args.epochs + 1):
        ...
        train_loss = trainLLM(args, model, device, train_loader, tokeniser, optimiser, epoch)
        scheduler.step()
        args.wandbrun.log({"loss_epoch": train_loss}, step=epoch)
        torch.cuda.empty_cache()

    print("Fine-tuning complete. Saving the model..")
    model.save_pretrained(f"checkpoint/LLM_model_{args.tag_FT}")
    tokeniser.save_pretrained(f"checkpoint/LLM_tokeniser_{args.tag_FT}")
