...
def main():
    ...
    parser.add_argument('--subset_proteins_4FT', type=float, default=0.5,
                        help='A floating point number between 0 and 1 (for percentage of available proteins), or an integer greater than 1 (number of proteins) - to use a subset of proteins for fine-tuning. The other half will be used for testing. If 1, then all proteins will be used for fine-tuning.')  
    ...
    if args.subset_proteins_4FT != 1:
        print("Using a subset of proteins for fine-tuning.")
        if args.subset_proteins_4FT < 1:
            train_protein_index = np.random.choice(range(train_protein.n_vars), int(args.subset_proteins_4FT*train_protein.n_vars), replace=False)
        else:
            train_protein_index = np.random.choice(range(train_protein.n_vars), int(args.subset_proteins_4FT), replace=False)
        test_protein_index = np.setdiff1d(range(train_protein.n_vars), train_protein_index)
        print(f"Using {len(train_protein_index)} proteins for fine-tuning and {len(test_protein_index)} proteins for testing.")
        train_protein = train_protein[:, train_protein_index]
        test_protein = test_protein[:, test_protein_index]
